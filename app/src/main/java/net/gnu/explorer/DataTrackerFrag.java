...
//	};
	...
	public DataTrackerFrag() {
		super();
		type = Frag.TYPE.TRAFFIC_STATS.ordinal();
	}
...
//		networkSpinner.setSelection(0);
		...
		appStatAdapter = new AppsAdapter(activity);

		trackListView = (ListView) v.findViewById(R.id.processes);
		trackListView.setFastScrollEnabled(true);

		trackListView.setScrollBarStyle(View.SCROLLBARS_OUTSIDE_OVERLAY);
		trackListView.setAdapter(appStatAdapter);
		//runnable.run();//loadlist(getContext());
		trackListView.setOnScrollListener(new AbsListView.OnScrollListener() {
...
//						txRate += appStats.bytesTxSincePrev * 1000000 / (appStats.elapsedTimeSincePrev / 1000);
	...
	class AppsAdapter extends ArrayAdapter<AppStats> implements OnClickListener, OnLongClickListener {
		...
		public AppsAdapter(final Context context) {
			super(context, R.layout.list_item_net, appStatsList);
		}
...
//									break;
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				final LayoutInflater inflater = (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);//. getLayoutInflater();
				convertView = inflater.inflate(R.layout.list_item_net, parent, false);
