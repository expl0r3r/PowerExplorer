...
public class PhotoFragment extends Frag implements View.OnTouchListener,
...
//			else
	...
	final public void showWait() {
        if (pd == null)
            pd = ProgressDialog.show(PhotoFragment.this.getContext(), "", getString(R.string.loading), true, true);
    }
...
//            return PhotoFragment.this.getContext();
//        }
//        @Override
...
//                    Toast.makeText( PhotoFragment.this.getContext(), html ? Html.fromHtml( s ) : s, Toast.LENGTH_LONG ).show();
//                }
//            }
//            return false;
//        }
...
//    }
	...
	private class LoaderThread extends Thread {
        ...
        LoaderThread(Uri u_, String name_) {
            ctx = PhotoFragment.this.getContext();
            u = u_;
            name_to_show = name_;
            //file_path = null;
...
//                if( ca.hasFeature( CommanderAdapter.Feature.LOCAL ) ) {
				...
				ContentResolver cr = (ContentResolver.SCHEME_CONTENT.equals(scheme))
					? PhotoFragment.this.getContext().getContentResolver() : null;
				if (!CURRENT_PATH.toLowerCase().endsWith("xml") && !CURRENT_PATH.toLowerCase().endsWith("svg")) {
					for (int b = 1; b < 0x80000; b <<= 1) {
                        try {
                            options.inSampleSize = b;
//                            if( ca != null )
//                                is = new BufferedInputStream(PhotoFragment.this.ca.getContent( u ));
//                            else 
							if (cr != null) {
                            ...
                            if (is != null)
								FileUtil.close(is);
							//PhotoFragment.this.ca.closeStream( is );
                        }
                        Log.w(TAG, "Cant decode stream to bitmap. b=" + b);
                    }
				}
				PhotoFragment.this.h.post(new Runnable() {
						@Override
						public void run() {
							if (CURRENT_PATH.toLowerCase().endsWith("xml") || CURRENT_PATH.toLowerCase().endsWith("svg")) {
								...
								PhotoFragment.this.setBitmapToView(bmp, name_to_show);
...
//                            throw e;
//                        } finally {
//                            if( PhotoFragment.this.ca != null && is != null ) 
//                                PhotoFragment.this.ca.closeStream( is );
...
//									@Override
//									public void run() {
//										PhotoFragment.this.setBitmapToView( bmp, name );
//										//bmp.recycle();
//									}
//								});                
//                            return;
...
//                    }
                ...
                PhotoFragment.this.h.post(new Runnable() {
						...
						public void run() {
							hideWait();
							Toast.makeText(PhotoFragment.this.getContext(), msgText != null ? msgText : ctx.getString(R.string.error), 
