...
// Sort the stats by the last time used
	...
	public static StringBuilder getSignature(Context context, String apkPath) {
				...
				if (applicationInfo != null) {
					...
					for (final Signature sig : arrSignatures) {
						...
						try {
							...
							sb.append("\nCertificate issuer: " + x509Cert.getIssuerDN());
...
//		}
	...
	public static void addShortcut(final Activity activity, final File f) {
        //Adding shortcut for MainActivity
        //on Home screen
        Intent shortcutIntent = new Intent(activity.getApplicationContext(),
										   activity.getClass());
        shortcutIntent.putExtra(ExplorerActivity.EXTRA_DIR_PATH, f.getAbsolutePath());
        shortcutIntent.setAction(Intent.ACTION_MAIN);
        shortcutIntent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
		if (f.isDirectory()) {
			shortcutIntent.putExtra(ExplorerActivity.EXTRA_MULTI_SELECT, true);
			shortcutIntent.putExtra(ExplorerActivity.EXTRA_SUFFIX, ".*");
		}
		...
		if (f.isFile()) {
			addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
							   Intent.ShortcutIconResource.fromContext(activity, ImageThreadLoader.getResId(f)));//R.mipmap.ic_launcher));
		} else {
			addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
							   Intent.ShortcutIconResource.fromContext(activity, R.drawable.myfolder72));
		}
